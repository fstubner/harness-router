...
| Version | Status             |
| ------- | ------------------ |
| 0.3.x   | supported (latest) |
| < 0.3   | no security fixes  |
...
1. **GitHub private vulnerability reporting** — preferred. Open the [Security tab](https://github.com/fstubner/harness-router/security/advisories/new) on the repo and submit a private advisory.
2. **Email** — `felix.stubner@gmail.com` with `[harness-router security]` in the subject line.

Please include:

- Affected version (`harness-router --version`)
...
- The published `harness-router` npm package and its `dist/` output.
- The MCP tools and prompts the server exposes.
- The `harness-router onboard` and `harness-router doctor` setup flows.
